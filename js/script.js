...
heartExcuseButton.addEventListener(
  ...
  () => {
    if (heartExcuseButton.classList.contains("on")) {
      ...
      heartExcuseText.classList.add("caption--color_variant_70");
    }
  },
  false
);
...
function openItem(evt, name) {
  var i, tab__body, tab__element;
  tab__body = document.getElementsByClassName("tab__body");
  for (i = 0; i < tab__body.length; i++) {
    tab__body[i].style.display = "none";
  }
  tab__element = document.getElementsByClassName("tab__element");
  for (i = 0; i < tab__element.length; i++) {
    tab__element[i].className = tab__element[i].className.replace(
      " active",
      ""
    );
  }
